...
import { FilterFilled, CaretRightOutlined } from "@ant-design/icons";
function MyInput({
  data,
  dataIndex,
  condition,
          ...
          switch (condition) {
            ...
            case "isContainsPhrase":
              return (
                ...
                xValue.includes(customInputLowerCase)
              );
            ...
            case "beginsWith":
              return (
                typeof xValue === "string" &&
                xValue.startsWith(customInputLowerCase)
              );
            case "endsWith":
              return (
                typeof xValue === "string" &&
                xValue.endsWith(customInputLowerCase)
              );
            default:
              return true; // If condition is not specified, return all data
          }
...
const FilterbyuniqItem = ({ dataIndex, setDataSource }) => {
  ...
  return (
    ...
    >
      <Button
        className="flex justify-between items-center hover:bg-sky-100 w-full"
        type="secondary"
      >
        <div className="flex items-center gap-2">
          <FilterFilled /> <p>Filter Text</p>
        </div>
        <CaretRightOutlined />
      </Button>
    </Popover>
