  ...
  const toggleColumnPinned = (dataIndex, side) => {
    if (side === "left") {
      if (leftPinnedColumns.includes(dataIndex)) {
        dispatch(
          SET_LEFT_PINNED_COLUMNS(
            leftPinnedColumns.filter((col) => col !== dataIndex)
          )
        );
      } else {
      ...
      if (rightPinnedColumns.includes(dataIndex)) {
        dispatch(
          SET_RIGHT_PINNED_COLUMNS(
            rightPinnedColumns.filter((col) => col !== dataIndex)
          )
        );
      }
    } else if (side === "right") {
...
const PinPopover = ({ dataIndex }) => {
  return (
    <Popover placement="right" content={<Content dataIndex={dataIndex} />}>
      <Button
        className="flex justify-between items-center hover:bg-sky-100"
        type="secondary"
      >
        <div className="flex items-center gap-2">
          <PushpinOutlined /> <p>Pin Column</p>
        </div>
        <CaretRightOutlined />
      </Button>
    </Popover>
